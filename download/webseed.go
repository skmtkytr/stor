...
// For single-file torrents, the URL points to the file directly.
// For multi-file torrents, the URL is a base directory.
func downloadPieceHTTP(ctx context.Context, baseURL string, tf *torrent.TorrentFile, pw PieceWork) ([]byte, error) {
	...
	if !storage.IsMultiFile(tf) {
		return httpRange(ctx, baseURL, offset, int64(pw.Length))
	}

	// Multi-file: piece may span files, download each segment
...
func httpRange(ctx context.Context, url string, offset, length int64) ([]byte, error) {
	...
	if resp.StatusCode != http.StatusPartialContent && resp.StatusCode != http.StatusOK {
		return nil, fmt.Errorf("webseed: HTTP %d for %s", resp.StatusCode, url)
	}
